...
import { LoadingSpinner, PulseLoader, SkeletonLoader } from './components/LoadingAnimations';
import { exportToExcel, exportToCalendar } from './utils/ExportUtils';
import './styles/ROTAStyles.css';
...
export default function ROTAScheduler() {
    ...
    return (
        <div className={`min-h-screen relative font-sans selection:bg-teal-100 selection:text-teal-700 ${isDarkMode ? 'bg-slate-900 text-slate-100' : 'bg-slate-50 text-slate-900'}`}>
            ...
            <div className="relative z-10">
                {notification && (
                    <motion.div 
                        ...
                        className={`fixed top-6 right-6 z-50 px-6 py-4 rounded-2xl shadow-2xl flex items-center gap-3 border glass-card ${notification.type === 'success' ? 'text-green-700' : 'text-red-700'
                        }`}>
            ...
            {showStatsModal && (
                <motion.div 
                    ...
                    animate={{ opacity: 1 }}
                    exit={{ opacity: 0 }}
                    className="fixed inset-0 bg-slate-900/40 backdrop-blur-sm flex items-center justify-center z-50 transition-all p-4"
                >
                    <AnimatedCard className={`p-8 w-full max-w-md shadow-2xl border ring-1 ring-black/5 ${isDarkMode ? 'bg-slate-900/80 border-slate-700/50' : 'bg-white/80 border-white/50'}`}>
                        <h3 className={`text-2xl font-black mb-6 flex items-center gap-3 gradient-text`}>
